...
class TurnToSpecificPoint(commands2.CommandBase):
    ...
    def execute(self) -> None:
        ...
        currentAngle = conversions.convertTalonFXUnitsToDegrees(
            self.wheel.directionMotor.getSelectedSensorPosition())
        ...
        if self.magnitude != 0.0:
            if (math.fabs(currentAngle - self.angle) 
                <= math.fabs(currentAngle - opposAngle)):
                # turn to the original angle
                if self.angle == 0.0:

                    if ((2048 * constants.ksteeringGearRatio) - self.units 
                        < self.wheel.directionMotor.getSelectedSensorPosition()):
                ...
                if self.angle == 0.0:
                    if ((2048 * constants.ksteeringGearRatio) 
                        - conversions.convertDegreesToTalonFXUnits(opposAngle) 
                        < self.wheel.directionMotor.getSelectedSensorPosition()):
        ...
        wpilib.SmartDashboard.putNumber(
            "Input to wheel -", self.units * constants.ksteeringGearRatio)
        
        wpilib.SmartDashboard.putNumber("Magnitude -", self.magnitude)
        
        wpilib.SmartDashboard.putNumber(
            "Difference fro 2048 -", 
            float((2048 * constants.ksteeringGearRatio) 
                  - self.wheel.directionMotor.getSelectedSensorPosition()))
